# go-boot-hertz 项目开发规范文档

go-boot-hertz 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 hertz Web 框架集成模块。本模块将 hertz 框架无缝集成到 go-boot 的 IoC 容器和自动配置体系中，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-hertz 是 go-boot 的 Web 层集成，将 hertz 作为 HTTP 服务器实现
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 hertz Engine 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 hertz 启动器
- 实现 `net.Server` 接口的 hertz 适配器
- 支持路由注册、中间件配置的声明式配置
...
- **Web 框架**：集成 hertz（`github.com/cloudwego/hertz`）
- **职责边界**：仅负责 hertz 集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 hertz 集成用法

### 2.2 go-boot-hertz 核心包结构

| 包 | 说明 | 接口定义 |
|---|------|----------|
| `hertz/` | hertz Engine 集成（自动配置、Bean 注册） | `hertz.Engine` |
| `server/` | hertz Server 适配器（实现 net.Server） | `server.HertzServer` |
| `router/` | hertz Router 集成（声明式路由配置） | `router.Router` |
| `middleware/` | hertz 中间件配置（日志、恢复、CORS 等） | 各类中间件函数 |

### 2.3 go-boot 核心包参考

go-boot-hertz 依赖 go-boot 的以下核心包：
...
go-boot-hertz 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：
...
#### 3.14.1 Engine 配置
- hertz Engine 通过 go-boot IoC 容器管理，使用自动配置注册
- Engine 配置项通过 `environment` 注入，支持配置文件和环境变量
- 常用配置项：
  - `hertz.host`: 监听地址（默认 `:8080`）
  - `hertz.port`: 监听端口（默认 `8080`）
  - `hertz.readTimeout`: 读取超时（默认 `3s`）
  - `hertz.idleTimeout`: 空闲超时（默认 `60s`）
  - `hertz.maxRequestBodySize`: 最大请求体大小（默认 `4MB`）

#### 3.14.2 路由注册规范
- 路由注册通过 `router.Router` 接口进行，支持声明式配置
- 路由分组使用业务领域划分，如 `/api/v1/users`、`/api/v1/orders`
- 路由处理函数签名：`func(ctx context.Context, c *app.RequestContext)`
- 路由处理函数应保持简洁，复杂逻辑委托给 Service 层

```go
// 良好 — 路由处理函数简洁，委托给 Service
...
#### 3.14.3 中间件开发规范
- 中间件签名：`func() app.HandlerFunc` 或 `func(next app.HandlerFunc) app.HandlerFunc`
- 中间件应按职责单一原则拆分，如日志、恢复、认证、CORS 等
- 中间件执行顺序：全局中间件 → 分组中间件 → 路由中间件
- 中间件中应避免阻塞操作，耗时操作应使用 goroutine 或异步处理
- 中间件必须正确处理错误传递，不应吞掉下游错误
...
- 使用 `c.Param()` 获取路径参数
- 使用 `c.Query()` 获取查询参数
- 使用 `c.GetPostForm()` 获取表单参数
- 使用 `c.Bind()` 绑定请求体到结构体
- 参数验证应在 Handler 层进行，使用 `validator` 标签
- 响应格式统一使用 JSON，包含 `code`、`message`、`data` 字段

#### 3.14.5 响应格式规范
- 成功响应：`{"code": 0, "message": "success", "data": {...}}`
- 错误响应：`{"code": -1, "message": "error message", "data": null}`
- 分页响应：`{"code": 0, "message": "success", "data": {...}, "total": 100, "page": 1, "size": 10}`
- 使用统一的响应结构体封装
...
#### 3.14.6 Server 适配器规范
- `HertzServer` 实现 `net.Server` 接口
- Server 启动和停止通过 go-boot 生命周期管理
- Server 配置通过 `environment` 注入
- Server 启动时自动注册路由和中间件
- Server 停止时优雅关闭，等待活跃请求完成
